...
func getTestFiles() string {
	systemdb := os.Getenv("FXDMX_TEST_ROOT")

	if systemdb == "" {
		home, _ := homedir.Dir()
		if home != "" {
			systemdb = path.Join(home, "fxdmx", "db", "system.db")
		}
	}
	return systemdb
}
...
func TestRoot_GetTestDBPaths_Successful(t *testing.T) {
	...
	if systemdb == "" {
		t.Fatal("The required FXDMX_TEST_ROOT environment variable is not set to the test database root path.  It should probably be $HOME/fxdmx/db/system.db")
	}
